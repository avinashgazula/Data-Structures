...
void dfs(lptr &t,lptr l1,int s,int v,int **AdjMatrix,int &c)
{
	lptr l=NULL;
	if(c==0)
	addll(t,v);
	...
	if(Degree(s,AdjMatrix,v)==0)
	{
		...
		while(t2!=NULL)
		{
			...
			{
				dfs(t,l1,s,t2->data,AdjMatrix,c);
				break;
			}
			t2=t2->next;
		}
	...
	else
	{
		...
		for(int i=0;i<s;i++)
		{
			if(i!=v&&AdjMatrix[v][i]==1)
			{
				...
				if(ct==1)
				{
					max=Degree(s,AdjMatrix,i);
					maxpos=i;
				}
				else if(Degree(s,AdjMatrix,i)>max)
				{
					max=Degree(s,AdjMatrix,i);
					maxpos=i;
				}
		...
		AdjMatrix[v][maxpos]=0;
		AdjMatrix[maxpos][v]=0;
		dfs(l,l1,s,maxpos,AdjMatrix,c);
		return;
	}
}
int main()
