...
int main()
{
	...
	cout<<endl;int k=0,m1=0;int v1[20],v2[20];
	int count=1;
	while(count<size)
	{
		m1=0;
		prim p=percolate_down(h,s);
		if(count==1)
		{
			cout<<p.e1<<" "<<p.e2<<endl;
			v1[k]=p.e1;v2[k++]=p.e2;
		...
		else
		{
			...
			for(i=0;i<k;i++)
			{
				if(v1[k]!=p.e1&&v2[k]!=p.e1&&(adj(AdjMatrix,v1[i],p.e1)||adj(AdjMatrix,v2[i],p.e1)))
				c1++;
				if(v2[k]!=p.e2&&v1[k]!=p.e2&&(adj(AdjMatrix,v1[i],p.e1)||adj(AdjMatrix,v2[i],p.e1)))
				c2++;
			}
			while((c1==0&&c2==0))
			{
				c1=0;c2=0;
				push(stk,p);
				p=percolate_down(h,s);
				for(i=0;i<k;i++)
				{
					if(adj(AdjMatrix,v1[i],p.e1)||adj(AdjMatrix,v2[i],p.e1))
					c1++;
					if(adj(AdjMatrix,v1[i],p.e1)||adj(AdjMatrix,v2[i],p.e1))
					c2++;
				}
